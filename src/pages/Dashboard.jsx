...
import { getLatestBriefing, triggerGeneration, pollStatus, getEmailBody, sendToActualBudget, setTokenGetter } from "../api";
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    getLatestBriefing()
      .then(res => {
        const transformed = transformBriefing(res.briefing_json);
  ...
  async function handleRefresh() {
    setGenerating(true);
    try {
      const { id } = await triggerGeneration();
      const poll = setInterval(async () => {
        try {
          ...
          if (status === "ready") {
            ...
            setBriefing(transformBriefing(res.briefing_json));
            setGenerating(false);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(165deg, #0a0a0f 0%, #0f1118 40%, #111827 100%)", color: "#e2e8f0", fontFamily: "'DM Sans', 'Helvetica Neue', sans-serif", padding: "24px", maxWidth: 900, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          <div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 6 }}>
              <p style={{ color: "#64748b", fontSize: 13, margin: 0 }}>{d.generatedAt} · Powered by Claude Haiku 4.5</p>
              <button onClick={handleRefresh} disabled={generating} style={{
                ...
                cursor: generating ? "not-allowed" : "pointer", display: "flex", alignItems: "center", gap: 4,
      ...
      <div style={{ textAlign: "center", padding: "32px 0 16px", opacity: loaded ? 0.4 : 0, transition: "opacity 1s ease 1.2s" }}>
        <div style={{ fontSize: 11, color: "#475569", letterSpacing: 1 }}>NEXT BRIEFING · 12:00 PM MIDDAY CHECK-IN</div>
      </div>
    </div>
  );
}
