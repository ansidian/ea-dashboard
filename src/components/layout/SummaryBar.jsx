...
export default function SummaryBar({ stats, loaded, urgentEmails, deadlinesToday, billsDueToday, modalContent }) {
  ...
  // #3 — next event context
  const eventLabel = (() => {
    if (!stats.nextEvent) return `event${stats.events !== 1 ? "s" : ""}`;
    const minAway = stats.nextEvent.minutesUntil;
    if (minAway <= 0) return `now · ${stats.nextEvent.title}`;
    if (minAway <= 60) return `in ${minAway}m · ${stats.nextEvent.title}`;
    return `event${stats.events !== 1 ? "s" : ""}`;
  })();

  const items = [
    // #1 — pulse when 3+ urgent emails
    stats.urgentEmails > 0 && {
      badgeKey: "emails",
      color: "#f38ba8",
      pulse: stats.urgentEmails >= 3,
      icon: emailIcon,
      value: stats.urgentEmails,
      label: `email${stats.urgentEmails !== 1 ? "s" : ""} need${stats.urgentEmails !== 1 ? "" : "s"} action`,
    },
    // #2 — overdue bills (separate from due-today)
    stats.billsOverdue > 0 && {
      badgeKey: "bills",
      color: "#f38ba8",
      icon: billIcon,
      value: stats.billsOverdue,
      label: `bill${stats.billsOverdue !== 1 ? "s" : ""} overdue`,
    },
    ...
    // #4 — deadline progress fraction
    stats.dueToday > 0 && {
      badgeKey: "deadlines",
      color: "#fab387",
      icon: deadlineIcon,
      value: stats.dueToday,
      label: stats.totalDeadlines > stats.dueToday
        ? `of ${stats.totalDeadlines} due today`
        : "due today",
    },
    // #3 — next event context
    stats.events > 0 && {
