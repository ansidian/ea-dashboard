...
export default function EmailBody({ email, model, onLoaded }) {
  ...
  return (
    <div
      ...
      className="animate-in fade-in border-t border-white/5 mt-3 pt-3.5"
    >
      {email.preview && (
        <div className="flex items-start gap-1.5 mb-3 p-2.5 px-3 bg-warning/[0.04] rounded-default border border-warning/[0.08]">
      ...
      {loadingBody ? (
        <div className="flex items-center gap-2 py-3">
          <div className="w-3.5 h-3.5 border-2 border-white/10 border-t-accent-light rounded-full animate-spin" />
        ...
        isHtml ? (
          ...
          <pre className="font-sans text-[13px] leading-relaxed text-text-secondary whitespace-pre-wrap break-words m-0 max-h-[70vh] overflow-y-auto">{body}</pre>
        )
      ) : (
